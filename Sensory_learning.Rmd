...
#### Optional To Do's
 1. Add hovers and lands to first visits and plots
 2. Add hovers and lands to total visits and plots
...
library(PMCMRplus) #freidman test post-hocs
library(boot)
library(readr)
library(lme4)
library(emmeans)
...
#selecting relevent columns from main data, renaming to standardize (in particular, the nearest columns, making them all "nearest", rather than "next nearest", as for the trials where nearest was possible)

first1<- LocVSound %>% select(Bat.ID, Scorer, Date, Time, Species, Test, Rewarded_loc, Rewarded_sound, first_hover, first_land, first_visit, first_hover_nearest = first_hover_nextnearest, first_land_nearest = first_land_nextnearest, first_visit_nearest = first_visit_nextnearest) 



first2 <- LocVSmell %>% select(Bat.ID, Scorer, Date, Time, Species, Test, Rewarded_loc, Rewarded_smell, first_hover, first_land, first_visit, first_hover_nearest = first_hover_nextnearest, first_land_nearest = first_land_nextnearest, first_visit_nearest = first_visit_nextnearest)   
...
first_choices$first_visit <- factor(first_choices$first_visit, levels = c("location", "smell", "sound", "neither"))


#simplified doc for Gerry: 
first_choices1<- first_choices %>%select(Bat.ID, Date, Species, Test, first_hover, first_land, first_visit, first_hover_nearest, first_land_nearest, first_visit_nearest)
#write_csv(first_choices1, "first_choices.csv")
```

#summarise first choices by species, test, add small value to 0 bars for plotting
```{r}
first_choices1 <- first_choices1[!is.na(first_choices1$first_visit), ] # remove empty rows / bats that never visited

first_visits_sum <- first_choices1 %>% group_by(Species, Test, first_visit) %>% summarise(n_visits= n()) %>% 
...
## Make new data frame with summary level data for first visits  for locVsound
```{r}
head(first_choices)

#pull out locvSound trials
fc_LocVSound<- first_choices[first_choices$Test=="LocVSound",]

#redo factor order, removes extra levels
fc_LocVSound$first_visit <- factor(fc_LocVSound$first_visit, levels = c("location", "sound", "neither"))  

#summarize by species into table
first_visits_LvS <- as.data.frame(table(fc_LocVSound$first_visit, fc_LocVSound$Species)) 
#name columns
...
#observed_LvS_AJ <- first_visits_LvS_wide$AJ
prob <- rep(1/3, 3) #expected number of visits if they are all equal
prob 
mult_test_LvS_AJ <- multinomial.test(observed_LvS_AJ, prob) #exact multinomial test
mult_test_LvS_AJ  ##0.05329


#####What kind of posthocs? Do A binomial test, with the treatment in question, and the sum of the other treatments, and then do a Holm correction: dif I use Holm correction, sig. value would be Holm = Target α / (n – rank + 1) 
# 1st rank = 0.05/ (3-1+1) = 0.0167 
# 2nd rank = 0.025
# 3rd rank = 0.05
...
#Holm: .0125 significance for first rank

# run all post-hocs for AJ LvS, put in list
LvS_AJ_binomials<-list(
mult_test_LvS_AJ =mult_test_LvS_AJ,
LvS_AJ_binomial_loc = multinomial.test(obs_LvS_AJ_loc, prob2) ,
LvS_AJ_binomial_sound =  multinomial.test(obs_LvS_AJ_sound, prob2) ,
LvS_AJ_binomial_neither =  multinomial.test(obs_LvS_AJ_neither, prob2)
) 
 
# tidy for outputting: 
# tidy output of list
# LvS_AJ_binomial_tidy<- map_dfr(LvS_AJ_binomials, tidy) 
# # names() pulls out names of list, bind_rows puts all the binomial tests in the list into a dataframe, bind_cols puts those all together
# LvS_AJ_binomial_tidy <- bind_cols(names(LvS_AJ_binomials),bind_rows(LvS_AJ_binomial_tidy))
# LvS_AJ_binomial_tidy 

#write_csv(LvS_AJ_binomial_tidy, "Mult test and posthocs - Loc V Sound- AJ.csv")

# Exact multinomial test and posthovs suggest suggests that This is more visits to Location that expected at random
...
#  multinomial.test(obs_LvS_L_neither, prob2) # p = 1

#write_csv(tidy(mult_test_LvS_L), "Mult test - Loc V Sound- L.csv")
...
mult_test_LvSm_AJ #p-value = 0.01387
...
#Holm correction: Holm correction: Target α / (n – rank + 1): 
# 1st rank = 0.05/ (3-1+1) = 0.0167 
# 2nd rank = 0.025
# 3rd rank = 0.05
Holm_3 <- c(0.05, 0.0167, 0.025, 0.05) #alpha values for full multinomial test and 3 posthoc tests with Holm correction

 # run all post-hocs for AJ LvSm, put in list
LvSm_AJ_binomials<-list(
  mult_test_LvSm_AJ      = mult_test_LvSm_AJ ,
 LvSm_AJ_binomial_loc    = multinomial.test(obs_LvSm_AJ_loc, prob2) ,
 LvSm_AJ_binomial_smell  = multinomial.test(obs_LvSm_AJ_smell, prob2) ,
LvSm_AJ_binomial_neither = multinomial.test(obs_LvSm_AJ_neither, prob2)
)

 
# tidy for outputting: 
# LvSm_AJ_binomial_tidy<- map_dfr(LvSm_AJ_binomials, tidy) 
# 
# # names() pulls out names of list, bind_rows puts all the binomial tests in the list into a dataframe, bind_cols puts those all together
# LvSm_AJ_binomial_tidy <- bind_cols(names(LvSm_AJ_binomials),bind_rows(LvSm_AJ_binomial_tidy))
# 
# #add column with holm adjusted alpha values
# LvSm_AJ_binomial_tidy <- bind_cols(LvSm_AJ_binomial_tidy, Holm_3)
# 
# #name columns
# colnames(LvSm_AJ_binomial_tidy) <- c("test", "p.value", "method", "alpha")
# LvSm_AJ_binomial_tidy
# #write output
# ##write_csv(LvSm_AJ_binomial_tidy, "Mult test and posthocs - Loc V Smell- AJ.csv")
...
prob <- rep(1/3, 3) #expected proportion of visits if they are all equal

mult_test_LvSm_L <- multinomial.test(observed_LvSm_L, prob) #exact multinomial test
mult_test_LvSm_L #p = 0.005639
...
# # of bats that flew to feeder first, # of bats that did not
obs_LvSm_L_loc<-c(8,2)
obs_LvSm_L_smell <- c(0,10)
obs_LvSm_L_neither <- c(2,8)

#Binomial posthocs with Holm corrections: .0125 significance for first rank
LvSm_L_binomials<-list(
  mult_test_LvSm_L        = mult_test_LvSm_L ,
  LvSm_L_binomial_loc     = multinomial.test(obs_LvSm_L_loc, prob2) ,
  LvSm_L_binomial_smell   = multinomial.test(obs_LvSm_L_smell, prob2) ,
  LvSm_L_binomial_neither = multinomial.test(obs_LvSm_L_neither, prob2) 
)

 
# # tidy for outputting: 
# LvSm_L_binomial_tidy<- map_dfr(LvSm_L_binomials, tidy) 
# 
# # names() pulls out names of list, bind_rows puts all the binomial tests in the list into a dataframe, bind_cols puts those all together
# LvSm_L_binomial_tidy <- bind_cols(names(LvSm_L_binomials),bind_rows(LvSm_L_binomial_tidy))
# 
# #add column with holm adjusted alpha values
# LvSm_L_binomial_tidy <- bind_cols(LvSm_L_binomial_tidy, Holm_3)
# 
# #name columns
# colnames(LvSm_L_binomial_tidy) <- c("test", "p.value", "method", "alpha" )
# LvSm_L_binomial_tidy
# #write output
# #write_csv(LvSm_L_binomial_tidy, "Mult test and posthocs - Loc V Smell- L.csv")

```
Can reject null hypothesis that Lophostoma went to the 3 cues first equally- went to location more
...
mult_test_SvSm_AJ # p-value = 1

#write_csv(tidy(mult_test_SvSm_AJ), "Mult test _ Sound V Smell- AJ.csv")
...
mult_test_SvSm_L # p-value = 0.6266


#write_csv(tidy(mult_test_SvSm_L), "Mult test _ Sound V Smell- L.csv")
# Exact multinomial test suggests that these are not different than random
# no post-hoc tests necessary
...
# Putting dataset all together, long
```{r}
...
## Version of long data, m_Alltests, with no "NA"s
```{r}
View(m_Alltests)
m_Alltests1 <- drop_na(m_Alltests)

Sensory_visits<- m_Alltests1 %>% mutate(cue = str_replace(cue, "pct_visits_loc", "location"), cue = str_replace(cue, "pct_visits_sound", "sound"), cue = str_replace(cue, "pct_visits_neither", "neither"), cue = str_replace(cue, "pct_visits_smell", "odor")) %>% select(-visits_prop)

#write_csv(Sensory_visits, "Sensory_visits.csv")
```

## Data split by Test
```{r}
SvSm <- m_Alltests1 %>% filter(Test == "SoundVSmell") 
LvSm <- m_Alltests1 %>% filter(Test == "LocVSmell") 
LvSo <- m_Alltests1 %>% filter(Test == "LocVSound") 
```

## Data split by Test and Species (in case I want it, don't think I need it)
```{r}
#repeat J in SvSm_AJ
...
splitTestSp<- list(SvSm_L, LvSm_AJ, LvSm_L, LvSo_AJ, LvSo_L)
```
...
- Quade tests may possibly be more powerful when # groups (k, "cue" here, <5). Right now haven't run.(quade tests aren't pipe friendly rn) See: https://cran.r-project.org/web/packages/PMCMR/vignettes/PMCMR.pdf
```{r}
#run Friedman test for all groups at the same time: 

#use version of m_alltests with no NAs: m_Alltests1
...
friedman_test(visits_counts ~ cue | Bat.ID) # format (y ~ groups | blocks)
friedman_all
...
##Friedman test post-hocs - Nemenyi tests
- Nemenyi's All-Pairs Comparisons Test
- rn, both lvSo and LvSm treatments were significant, so post-hocs on them
```{r}
#Post hocs for Friedman test?: Nemenyi or Conover tests (see: https://cran.r-project.org/web/packages/PMCMR/vignettes/PMCMR.pdf). Choosing Nemenyi bc more conservative/was the first one I came across. 

#splitTestSp

#trying to figure out how to run all Nemenyi tests together (not pipe friendly), works ok, but hard to figure out what each one refers to, bc no names, will fix later!: 
NM <- lapply(splitTestSp, function(df) {
   NM1<- frdAllPairsNemenyiTest(visits_counts ~ cue | Bat.ID, data =df)
    NM1
})

#in the meantime, doing it row by row. 
#uses PMCMR package- doesn't play well with dplyr for some reason. 
Nemenyi_LvSm_AJ <- frdAllPairsNemenyiTest(visits_counts ~ cue | Bat.ID, data = LvSm_AJ) # format (y ~ groups | blocks)
Nemenyi_LvSm_L <- frdAllPairsNemenyiTest(visits_counts ~ cue | Bat.ID, data = LvSm_L) # format (y ~ groups | blocks)
Nemenyi_LvSo_AJ <- frdAllPairsNemenyiTest(visits_counts ~ cue | Bat.ID, data = LvSo_AJ) # format (y ~ groups | blocks)
Nemenyi_LvSo_L <- frdAllPairsNemenyiTest(visits_counts ~ cue | Bat.ID, data = LvSo_L) # format (y ~ groups | blocks)
...
#list these 
NemenyiList <- list(Nemenyi_LvSm_AJ = Nemenyi_LvSm_AJ, Nemenyi_LvSm_L =Nemenyi_LvSm_L, Nemenyi_LvSo_AJ = Nemenyi_LvSo_AJ, Nemenyi_LvSo_L = Nemenyi_LvSo_L)
NemenyiList

#summaries
 Summary_Nemenyi<- lapply(NemenyiList, summary )
 Summary_Nemenyi

# valuedataframes from each dataset in list, put into new list
Nemenyi_p.vals<- lapply(NemenyiList, function(x) x$p.value)

#write these to csv.s (would be nice one below another in the same CSV, but beggars/choosers)
#mapply(write.csv, Nemenyi_p.vals, file=paste0(names(Nemenyi_p.vals), '.csv'))
...
# lapply(NemenyiList, frdAllPairsNemenyiTest(visits_counts ~ cue | Bat.ID)) #can't find the right object


#Could make this into a tidyer dataframe (no tidy for these objects)
#pull out p.values, melt to long, 
#pull out statistic, melt to long
#add object name 

# Nemenyi_LvSm_AJ$model
# nemen.df <- left_join(
# melt(Nemenyi_LvSm_AJ$p.value, value.name = "p.value"),
# melt(Nemenyi_LvSm_AJ$statistic, value.name = "statistic") 
...
glmm.nb_bytest<- lapply(split(m_Alltests1,m_Alltests1$Test),
       glmer.nb,formula=visits_counts ~ cue*Species + (1|Bat.ID))
glmm.nb_bytest
# deviance/ df.resid  = 7.08, 6.23, 6.03 respectively (still severe overdispersion?)
# a little better, but not a ton? Test formally:


##  Zero- inflated:
glmm.p.zi.bytest<- lapply(split(m_Alltests1,m_Alltests1$Test),
       glmmTMB,formula=visits_counts ~ cue*Species + (1|Bat.ID), ziformula = ~1 , family = "poisson")
glmm.p.zi.bytest

# just have to update glmmTMM
...
## libraries that check for overdispersion
library(RVAideMemoire)
library(AER)
library(DHARMa)
citation("DHARMa")

## going through the DHARMa workflow: 
https://cran.r-project.org/web/packages/DHARMa/vignettes/DHARMa.html

#model objects
g.p.LVSm <- glmm_bytest$LocVSmell #poisson
g.nb.LVSm<- glmm.nb_bytest$LocVSmell #negative binomial

#simulation of residuals
simulationOutput.p <- simulateResiduals(fittedModel = g.p.LVSm)
plot(simulationOutput.p) 
simulationOutput.p #significant fit issues- seemingly overdispersed

simulationOutput.nb <- simulateResiduals(fittedModel = g.nb.LVSm)
plot(simulationOutput.nb) 
simulationOutput.nb #no significant fit issues

#targeted overdispersion tests

#Test 1 - A non-parametric test on the simulated residuals
disp.test.1.p<-testDispersion(simulationOutput.p)
disp.test.1.p #NS

disp.test.1.nb<-testDispersion(simulationOutput.nb)
disp.test.1.nb #p=0.088 , slightly significant. 


#Test 2 - A non-parametric overdispersion test on the re-fitted residuals.

simulationOutput.p2 <- simulateResiduals(fittedModel = g.p.LVSm, refit = T, n = 20)
disp.test.2.p <- testDispersion(simulationOutput.p2)
disp.test.2.p  #low p.value: 2.2*E-16

simulationOutput.nb2 <- simulateResiduals(fittedModel = g.nb.LVSm, refit = T, n = 20)
disp.test.2.nb <-testDispersion(simulationOutput.nb2)
disp.test.2.nb #p=0.5

# I don't know why these don't seem to agree. 


# Test for Zero-inflation 
zeroinflate.p<- testZeroInflation(simulationOutput.p)
zeroinflate.nb<- testZeroInflation(simulationOutput.nb)
zeroinflate.p
zeroinflate.nb

## Model as Zero inflated?
library(glmmTMB)

#sample code


dispersiontest(glmm_bytest$LocVSmell)
testDispersion(glmm_bytest$LocVSmell)
...
disperseion_locvSmell<- overdisp.glmer(glmm_bytest$LocVSmell)
...
# Exploratory plots showing individual variation
```{r}
########
...
# ggsave("hist_AJ_Lopho_count_visits.jpg")
...
hist_AJ_Lopho_prop  <- ggplot(m_Alltests, aes(visits_prop, fill = Species)) +
 ...
 # scale_x_log10() +
  facet_grid(cols= vars (Species), margins=TRUE, scales="free_y", rows=vars(Test))
hist_AJ_Lopho_prop 
# ggsave("hist_AJ_Lopho_prop_visits.jpg")
...
scale_fill_discrete(name = "cue", labels = c("Location", "Sound", "Control", "Smell"))
...
theme(axis.text.x = element_text(angle = 90, size = 5)) +
facet_grid(cols= vars (Species),  rows=vars(Test), scales = "free") +
